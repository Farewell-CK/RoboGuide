...
class Enumerator {
    ...
    public Enumerator(Context context, DeviceListener listener){
        ...
        // ACTION_USB_PERMISSION's result broadcast is delivered by the system on our own
        // PendingIntent (not by another app process), so it must be registered EXPORTED or the
        // system's delivery is silently dropped on API 33+, breaking permission grants that
        // happen while the app is already running (e.g. a live OTG hot-plug).
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            context.registerReceiver(mBroadcastReceiver, new IntentFilter(UsbUtilities.ACTION_USB_PERMISSION), Context.RECEIVER_EXPORTED);
            context.registerReceiver(mBroadcastReceiver, new IntentFilter(UsbManager.ACTION_USB_DEVICE_ATTACHED), Context.RECEIVER_NOT_EXPORTED);
            context.registerReceiver(mBroadcastReceiver, new IntentFilter(UsbManager.ACTION_USB_DEVICE_DETACHED), Context.RECEIVER_NOT_EXPORTED);
        } else {
            ContextCompat.registerReceiver(context, mBroadcastReceiver, new IntentFilter(UsbUtilities.ACTION_USB_PERMISSION), ContextCompat.RECEIVER_EXPORTED);
