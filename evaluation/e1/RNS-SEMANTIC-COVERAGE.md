...
| R2 | 跨楼层/楼梯可达性差异 | ep51 same_floor=false, Δh=2.87 | 无全局差异（any agent 皆可达）→ NOT_NEEDED 为全局语义；floor 属 LOCAL_HOW | mobility.navigate@v1 零属性即可忠实 | EXPRESSIBLE（零属性恰是正确表达） | false | pddl_sim_state.py:412；两 agent yaml 无 locomotion 差异 |
...
| R9 | 互斥持物（同对象不能两人同持） | manipulation 族 | RESOURCE（对象级互斥）/LOCAL_HOW（sim 内强制） | 官方 Independent 集合内单持者即可；Node-local `local_locks` 语义足够 | EXPRESSIBLE（Independent 范围内） | false | pddl yaml；node.toml local_locks |
