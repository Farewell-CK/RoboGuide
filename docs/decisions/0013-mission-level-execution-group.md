...
- Mission-level Group/TaskExecution 事件使用新的 `domain.EventPayload.json/v2`；由于 Runtime
  execution identity 纳入 Task，Controller checkpoint 使用 `roboguide.controller-checkpoint/v3`
  版本标记，旧投影不得静默恢复。
