...
void OctomapGenerator<CLOUD, OCTREE>::insertPointCloud(const pcl::PCLPointCloud2::Ptr& cloud, const Eigen::Matrix4f& sensorToWorld)
{
  ...
  //Don't perform that. VoxelGrid will average all points in one voxel.
  ...
  for(typename CLOUD::const_iterator it = pcl_cloud.begin(); it != pcl_cloud.end(); ++it)
  {
    ...
    if (!std::isnan(it->x) && !std::isnan(it->y) && !std::isnan(it->z))
    {
      ...
      if (dist > valid_max_ || dist < valid_min_) continue;

      // Check if the point is in the ray casting range
      if(dist <= max_range_) // Add to a point cloud and do ray casting later all together
      {
        #pragma omp critical
        {
        ...
        octomap::point3d new_end = origin + direction * (max_range_+1);
        #pragma omp critical
        {
  ...
  int endpoint_count = raycast_cloud.size(); // total number of endpoints inserted
  ...
  updateColorAndSemantics(&pcl_cloud, origin);
  // updates inner node occupancy and colors
  // if(endpoint_count > 0)
  //   octomap_.updateInnerOccupancy();
  if (global_crop_){
    std::vector<octomap::OcTreeKey> to_be_removed;
    for (typename OCTREE::leaf_iterator x=octomap_.begin_leafs(); x!=octomap_.end_leafs(); ++x){
      if (origin.distance(x.getCoordinate())>max_range_) to_be_removed.push_back(x.getKey());
    }
    for (auto x: to_be_removed) octomap_.deleteNode(x);
  }
}

template<>
void OctomapGenerator<PCLColor, ColorOcTree>::updateColorAndSemantics(PCLColor* pcl_cloud, octomap::point3d origin)
{
  for(PCLColor::const_iterator it = pcl_cloud->begin(); it < pcl_cloud->end(); it++)
  {
    if (!std::isnan(it->x) && !std::isnan(it->y) && !std::isnan(it->z))
    {
      float dist = sqrt((it->x - origin.x())*(it->x - origin.x()) + (it->y - origin.y())*(it->y - origin.y()) + (it->z - origin.z())*(it->z - origin.z()));
      // Check if the point is in max_range
      if (dist < valid_min_ || dist > max_range_) continue;
...
template<>
void OctomapGenerator<PCLSemanticsMax, SemanticsOctreeMax>::updateColorAndSemantics(PCLSemanticsMax* pcl_cloud, octomap::point3d origin)
{
  #pragma omp parallel for
  for(PCLSemanticsMax::const_iterator it = pcl_cloud->begin(); it < pcl_cloud->end(); it++)
  {
    if (!std::isnan(it->x) && !std::isnan(it->y) && !std::isnan(it->z))
    {
      ...
      // int __rgb = sem.semantic_color.r*256*256 + sem.semantic_color.g*256 + sem.semantic_color.b;
      // if (octomap::cmaps.find(__rgb)==octomap::cmaps.end()){
      //   printf("read (%d,%d,%d,%f)\n", sem.semantic_color.r, sem.semantic_color.g, sem.semantic_color.b, sem.confidence);
      //   exit(0);
      // }
...
template<>
void OctomapGenerator<PCLSemanticsBayesian, SemanticsOctreeBayesian>::updateColorAndSemantics(PCLSemanticsBayesian* pcl_cloud, octomap::point3d origin)
{
  #pragma omp parallel for
