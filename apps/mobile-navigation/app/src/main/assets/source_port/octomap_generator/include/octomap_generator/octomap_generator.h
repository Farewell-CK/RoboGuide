...
class OctomapGenerator: public OctomapGeneratorBase
{
  public:
    ...
    virtual void setDistRange(float min_range, float max_range){valid_min_=min_range; valid_max_=max_range;}
    
    virtual void setGlobalCrop(bool global_crop){global_crop_ = global_crop;};
  ...
  protected:
    ...
    float valid_min_, valid_max_; /// Valid range of distance
    bool global_crop_;
    float raycast_range_; ///<Max range for points to perform raycasting to free unoccupied space
    void updateColorAndSemantics(CLOUD* pcl_cloud, octomap::point3d origin);
