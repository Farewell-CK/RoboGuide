...
extern "C" JNIEXPORT jlong JNICALL
Java_com_elabrador_mobilenavigation_NativeNeuronShim_nativeRunProfiled(
        JNIEnv* env, jclass, jlong handle, jobject inputBuffer, jobject outputBuffer,
        jlongArray timings) {
    if (timings != nullptr && env->GetArrayLength(timings) < 3) {
        throwIllegalState(env, "Neuron timing buffer must contain at least three elements");
        return -1;
    }
    ...
    const int64_t uploadStarted = monotonicNanos();
    ...
    const int64_t downloaded = monotonicNanos();
    if (status != 0) {
        throwIllegalState(env, "Neuron output download failed: " + std::to_string(status));
        return -1;
    }
    if (timings != nullptr) {
        const jlong stages[3] = {started - uploadStarted, elapsed,
                                downloaded - started - elapsed};
        env->SetLongArrayRegion(timings, 0, 3, stages);
    }
...
Java_com_elabrador_mobilenavigation_NativeNeuronShim_nativeRun(
        JNIEnv* env, jclass type, jlong handle, jobject inputBuffer, jobject outputBuffer) {
    return Java_com_elabrador_mobilenavigation_NativeNeuronShim_nativeRunProfiled(
            env, type, handle, inputBuffer, outputBuffer, nullptr);
}
