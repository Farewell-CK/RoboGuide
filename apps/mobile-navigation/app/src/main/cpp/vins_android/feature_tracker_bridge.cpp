...
Java_com_elabrador_mobilenavigation_NativeVinsFeatureTracker_nativeSetThreads(
        JNIEnv*, jclass, jint threads) {
    cv::setNumThreads(std::max(1, std::min(8, static_cast<int>(threads))));
}

extern "C" JNIEXPORT jstring JNICALL
Java_com_elabrador_mobilenavigation_NativeVinsFeatureTracker_nativeBenchmarkThreads(JNIEnv* env, jclass) {
    const int previous = cv::getNumThreads();
    std::ostringstream report;
    report << "TRACKER_THREADS previous=" << previous << '\n';
    cv::Mat source(480, 640, CV_8UC1);
    cv::RNG random(20260907);
    random.fill(source, cv::RNG::UNIFORM, 0, 256);
    cv::GaussianBlur(source, source, cv::Size(5, 5), 1.0);
    cv::Mat target;
    cv::Mat transform = (cv::Mat_<double>(2, 3) << 1, 0, 2.25, 0, 1, -1.5);
    cv::warpAffine(source, target, transform, source.size());
    std::vector<cv::Point2f> referenceFeatures, referenceFlow;
    std::vector<uchar> referenceStatus;
    std::vector<float> referenceError;
    for (int threads : {1, 2, 4, 8}) {
        cv::setNumThreads(threads);
        std::vector<double> times;
        int differences = 0;
        for (int iteration = 0; iteration < 35; ++iteration) {
            std::vector<cv::Point2f> features, flow;
            std::vector<uchar> status;
            std::vector<float> error;
            auto started = std::chrono::steady_clock::now();
            cv::goodFeaturesToTrack(source, features, 150, 0.01, 25);
            cv::calcOpticalFlowPyrLK(source, target, features, flow, status, error, cv::Size(21,21), 3);
            auto ended = std::chrono::steady_clock::now();
            if (threads == 1 && iteration == 0) {
                referenceFeatures = features; referenceFlow = flow;
                referenceStatus = status; referenceError = error;
            }
            if (features != referenceFeatures || flow != referenceFlow
                    || status != referenceStatus || error != referenceError) ++differences;
            if (iteration >= 5) times.push_back(std::chrono::duration<double, std::milli>(ended-started).count());
        }
        std::sort(times.begin(), times.end());
        report << "threads=" << threads << " median_ms=" << times[times.size()/2]
               << " p90_ms=" << times[times.size()*9/10] << " differing_runs=" << differences << '\n';
    }
    cv::setNumThreads(previous);
