...
final class ReadyAStar {
    ...
    ReadyAStar(int[] start,int[] goal,int[][] map,Set<Long> obstacles,Heuristic heuristic,double alpha){this.start=start;this.goal=goal;this.map=map;this.obstacles=obstacles;this.heuristic=heuristic;this.alpha=alpha;rows=map.length;cols=map[0].length;}
    List<int[]> searching(){
