...
public final class SemanticBenchmarkInstrumentation extends Instrumentation {
    ...
    private void runBenchmarks() throws Exception {
        ...
        if ("decode-parallel".equals(mode)) {
            ...
            FloatBuffer floats = logits.asFloatBuffer();
            java.util.Random random = new java.util.Random(93);
            for (int i = 0; i < floats.capacity(); i++) floats.put(i, random.nextFloat() * 30 - 15);
            int[] classes = new int[19];
            for (int i=0;i<19;i++) classes[i]=i;
            int[] expected = new int[plane], actual = new int[plane];
            float[] expectedConfidence = new float[plane], actualConfidence = new float[plane];
            long[] serial = new long[30], parallel = new long[30];
            for (int i=0;i<30;i++) {
                long start=SystemClock.elapsedRealtimeNanos();
                NativeOctomap.nativeDecodePidNet(logits,19,plane,classes,expected,expectedConfidence);
                long middle=SystemClock.elapsedRealtimeNanos();
                NativeOctomap.nativeDecodePidNetParallel(logits,19,plane,classes,actual,actualConfidence,true);
                serial[i]=middle-start; parallel[i]=SystemClock.elapsedRealtimeNanos()-middle;
                if (!Arrays.equals(expected,actual) || !Arrays.equals(expectedConfidence,actualConfidence))
                    throw new AssertionError("Parallel decode mismatch");
            }
            Arrays.sort(serial); Arrays.sort(parallel);
            Log.i(TAG,"DECODE_PARALLEL differences=0 serial_ms="+serial[15]/1e6+" parallel_ms="+parallel[15]/1e6);
            return;
        }
        if ("cloud-cache".equals(mode)) {
            verifyCloudCache();
            return;
        }
        if ("tracker-threads".equals(mode)) {
            Log.i(TAG, NativeVinsFeatureTracker.nativeBenchmarkThreads());
            return;
        }
    ...
    private void verifyCloudCache() {
        int count = 128 * 128;
        float[] pixels = new float[count * 2], depths = new float[count];
        float[] expected = new float[count * 3], actual = new float[count * 3];
        float[] coefficients = {-0.05382481f, 0.059774883f, -0.0006865871f,
                0.0005905421f, -0.019221678f};
        for (int i = 0; i < count; i++) {
            pixels[i * 2] = (i % 128) * 640 / 128;
            pixels[i * 2 + 1] = (i / 128) * 480 / 128;
            depths[i] = 0.101f + (i % 65500) * 0.001f;
        }
        for (int scene = 0; scene < 4; scene++) {
            int model = scene % 2 == 0 ? 2 : 4;
            float fx = 387.5f + scene;
            long[] oldTimes = new long[20], newTimes = new long[20];
            for (int run = 0; run < 20; run++) {
                long start = SystemClock.elapsedRealtimeNanos();
                NativeRealSense.nativeDeprojectPixels(640,480,319.0f,242.8f,fx,386.9f,
                        model,coefficients,pixels,depths,expected);
                long middle = SystemClock.elapsedRealtimeNanos();
                NativeRealSense.nativeDeprojectPixelsCached(640,480,319.0f,242.8f,fx,386.9f,
                        model,coefficients,pixels,depths,actual,true);
                long end = SystemClock.elapsedRealtimeNanos();
                oldTimes[run] = middle - start; newTimes[run] = end - middle;
                for (int i = 0; i < actual.length; i++) {
                    if (Float.floatToRawIntBits(actual[i]) != Float.floatToRawIntBits(expected[i]))
                        throw new AssertionError("Cloud cache differs scene="+scene+" index="+i);
                }
            }
            Arrays.sort(oldTimes); Arrays.sort(newTimes);
            Log.i(TAG, "CLOUD_CACHE scene="+scene+" bit_differences=0 sdk_ms="
                    + oldTimes[10]/1e6+" cached_ms="+newTimes[10]/1e6);
        }
    }

    private void runNeuronStages() throws Exception {
        verifyNativePidNetPreprocess();
        File model = new File(getTargetContext().getFilesDir(),
                BuildConfig.SEMANTIC_MODEL_ASSET);
        if (!model.isFile()) throw new IllegalStateException("Missing NPU model: " + model);
        final int width = BuildConfig.SEMANTIC_MODEL_WIDTH;
        final int height = BuildConfig.SEMANTIC_MODEL_HEIGHT;
        final int plane = BuildConfig.SEMANTIC_OUTPUT_WIDTH * BuildConfig.SEMANTIC_OUTPUT_HEIGHT;
        ByteBuffer input = ByteBuffer.allocateDirect(3 * width * height * 4)
                .order(ByteOrder.nativeOrder());
        ByteBuffer output = ByteBuffer.allocateDirect(19 * plane * 4)
                .order(ByteOrder.nativeOrder());
        byte[] rgb = new byte[640 * 480 * 3];
        for (int i = 0; i < rgb.length; i++) rgb[i] = (byte) (i * 37 + 11);
        int[] classes = new int[19];
        for (int i = 0; i < classes.length; i++) classes[i] = i;
        int[] mask = new int[plane];
        float[] confidence = new float[plane];
        long[] timings = new long[3];
        long handle = NativeNeuronShim.nativeCreate(model.getAbsolutePath(), true);
        try {
            for (int iteration = 0; iteration < 35; iteration++) {
                long start = SystemClock.elapsedRealtimeNanos();
                NativeOctomap.nativePreparePidNet(rgb, 640 * 3, 0, 0, 640, 480,
                        width, height, input);
                long prepared = SystemClock.elapsedRealtimeNanos();
                NativeNeuronShim.nativeRunProfiled(handle, input, output, timings);
                long inferred = SystemClock.elapsedRealtimeNanos();
                NativeOctomap.nativeDecodePidNet(output, 19, plane, classes, mask, confidence);
                long decoded = SystemClock.elapsedRealtimeNanos();
                Log.i(TAG, String.format(Locale.US,
                        "NEURON_BENCH iteration=%d warmup=%s prep=%.3f upload=%.3f invoke=%.3f "
                                + "download=%.3f decode=%.3f total=%.3f ms synthetic_input=true",
                        iteration, iteration < 5, (prepared - start) / 1e6,
                        timings[0] / 1e6, timings[1] / 1e6, timings[2] / 1e6,
                        (decoded - inferred) / 1e6, (decoded - start) / 1e6));
            }
        } finally {
            NativeNeuronShim.nativeDestroy(handle);
        }
    }
