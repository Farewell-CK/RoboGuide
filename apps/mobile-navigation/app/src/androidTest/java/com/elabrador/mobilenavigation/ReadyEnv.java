...
/** Direct port of local_planning.env.ReadyEnv. */
final class ReadyEnv {
    ...
    void obsMapSet(int rows, int cols, int[][] values) {
        ...
        for(int i=0;i<xRange;i++) for(int j=0;j<yRange;j++) if(isObstacleCost(values[i][j])) obstacles.add(ReadyAStar.key(i,j));
    }
