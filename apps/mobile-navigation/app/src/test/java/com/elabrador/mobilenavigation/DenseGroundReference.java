 ...
 */
final class DenseGroundReference {
    ...
    private final int[] observedCells;
    private int observedCount;

    DenseGroundReference(int width, int height) {
        this.width = width;
        this.height = height;
        ...
        observed = new boolean[cellCount];
        observedCells = new int[cellCount];
    ...
    void observe(int cell, float relativeHeight) {
        ...
        if (bin >= 0) {
            heightBinCells[bin * cellCount + cell] = true;
            if (!observed[cell]) {
                observed[cell] = true;
                observedCells[observedCount++] = cell;
            }
        }
    }
