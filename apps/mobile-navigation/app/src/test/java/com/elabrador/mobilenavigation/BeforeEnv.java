...
/** Direct port of local_planning.env.BeforeEnv. */
final class BeforeEnv {
    ...
    void obsMapSet(int rows, int cols, int[][] values) {
        ...
        for(int i=0;i<xRange;i++) for(int j=0;j<yRange;j++) if(isObstacleCost(values[i][j])) obstacles.add(BeforeAStar.key(i,j));
    }
