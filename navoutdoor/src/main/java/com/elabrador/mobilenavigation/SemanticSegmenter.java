...
final class SemanticSegmenter implements AutoCloseable {
    ...
    static final class Result {
        ...
        Result(boolean isNotWalkable, String label, float areaRatio, long inferenceMillis,
            ...
            this(isNotWalkable, label, areaRatio, inferenceMillis, backend, leftCost,
                    centerCost, rightCost, obstacleDistance, semanticPointCount,
        ...
        Result withMapPose(VinsMono.Pose pose) {
    ...
    private Runnable queuedInference;
    ...
    private final long[] neuronTimings = new long[3];
    private long neuronFrameCount;
    private long inferenceLogCount;
    private long mapRateStarted;
    private long mapRateCount;
    private long mapStageLogCount;
    ...
    private final LocalMapStabilizer localMapStabilizer = new LocalMapStabilizer();
    private volatile OctomapSnapshot latestOctomap;
    ...
    void resetVinsState() {
        ...
        synchronized (inputQueueLock) { queuedInference = null; }
    ...
    void submitRgb(byte[] rgb, int width, int height, int stride,
        ...
        synchronized (inputQueueLock) {
            ...
            // Never queue an unbounded sequence of old camera frames.
            queuedInference = () -> {
                try {
    ...
    private Result runPidNetInference(byte[] depth, int depthWidth, int depthHeight,
        ...
        if (USE_NEURON) {
            ...
            NativeNeuronShim.nativeRunProfiled(handle, liteRtInputBytes, liteRtOutputBytes,
                    neuronTimings);
        } else {
        ...
        long decoded = SystemClock.elapsedRealtimeNanos();
        if (USE_NEURON && ++neuronFrameCount % 30L == 0L) {
            Log.i(TAG, String.format(Locale.US,
                    "NEURON_STAGES prep=%.3fms upload=%.3fms invoke=%.3fms download=%.3fms "
                            + "decode=%.3fms inference_stage=%.3fms",
                    (prepared - started) / 1_000_000.0,
                    neuronTimings[0] / 1_000_000.0, neuronTimings[1] / 1_000_000.0,
                    neuronTimings[2] / 1_000_000.0, (decoded - inferred) / 1_000_000.0,
                    (decoded - started) / 1_000_000.0));
        }
        if (++inferenceLogCount % 30L == 0L) {
            Log.i(TAG, String.format(Locale.US,
                    "inference returned backend=%s elapsed=%.1fms",
                    backend, (inferred - started) / 1_000_000.0));
        }
    ...
    private Result finishInference(int[] mask, float[] confidence, int maskWidth,
        ...
        // Faster segmentation may finish before VINS publishes the bracketing pose.
        // Wait only on the map worker; never substitute a later/current pose.
        long poseDeadline = SystemClock.elapsedRealtimeNanos() + TimeUnit.MILLISECONDS.toNanos(250);
        while (framePose == null && !closed && generation == vinsGeneration.get()
                && SystemClock.elapsedRealtimeNanos() < poseDeadline) {
            Thread.sleep(5);
            framePose = vinsPoseHistory.at(frameTimestampSeconds);
        }
        if (framePose == null || closed || generation != vinsGeneration.get()) {
            Log.i(TAG, "semantic map skipped: no matching frame pose t=" + frameTimestampSeconds);
            return null;
        }
        long poseReady = SystemClock.elapsedRealtimeNanos();
        ...
        if (octomapHandle != 0L && leafCount > 0) {
            ...
            LocalMapStabilizer.Snapshot stable = localMapStabilizer.update(
                    grid.cost, projectionPose, observedAt, frameTimestampSeconds);
            localCostGrid = stable.costs;
            for (int i = 0; i < localCostGrid.length; i++) {
                if (localCostGrid[i] != grid.cost[i]) mapHeldCells++;
            }
            if (generation != vinsGeneration.get() || closed) return null;
        ...
        if (mapRateStarted == 0L) mapRateStarted = gridBuilt;
        else if (++mapRateCount == 30L) {
            Log.i(TAG, String.format(Locale.US,
                    "MAP_THROUGHPUT fps=%.3f valid_updates=%d frame_age_ms=%.1f pose_wait_ms=%.1f",
                    mapRateCount * 1e9 / (gridBuilt - mapRateStarted), mapRateCount,
                    System.currentTimeMillis() - frameTimestampSeconds * 1000.0,
                    (poseReady - cloudBuilt) / 1e6));
            mapRateStarted = gridBuilt;
            mapRateCount = 0;
        }
        ...
        if (++mapStageLogCount % 30L == 0L) {
            Log.i(TAG, String.format(Locale.US,
                    "stages total=%dms prep=%d model=%d decode=%d cloud=%d octomap=%d grid=%d pose_wait=%d "
                            + "frame_pose_delta=%.3fm yaw_delta=%.2fdeg points=%d leaves=%d known=%d "
                            + "frame_ground=%.3fm/%d/%d ground=%.3fm support=%d positive=%d candidates=%d cleared=%d map_held=%d",
                    millis(gridBuilt - started), millis(prepared - started),
                    millis(inferred - prepared), millis(decoded - inferred),
                    millis(cloudBuilt - decoded), millis(mapInserted - poseReady),
                    millis(gridBuilt - mapInserted), millis(poseReady - cloudBuilt), poseDelta, yawDelta,
