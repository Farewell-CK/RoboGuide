...
final class SemanticPointCloud {
    private static final ThreadLocal<long[]> cacheState = ThreadLocal.withInitial(() -> new long[2]);
    ...
    static Data generate(int[] mask, float[] confidence, int width, int height,
        ...
        long[] cache = cacheState.get();
        boolean useCache = "PIDNET_NEURON".equals(BuildConfig.SEMANTIC_MODEL_KIND) && cache[1] == 0;
        NativeRealSense.nativeDeprojectPixelsCached(
                intrinsic.getWidth(), intrinsic.getHeight(),
                intrinsic.getPpx(), intrinsic.getPpy(), intrinsic.getFx(), intrinsic.getFy(),
                intrinsic.getModel().value(), intrinsic.getCoeffs(), pixels, depths, xyz, useCache);
        if (useCache && (++cache[0] <= 5 || cache[0] % 120 == 0)) {
            float[] reference = new float[xyz.length];
            NativeRealSense.nativeDeprojectPixels(intrinsic.getWidth(), intrinsic.getHeight(),
                    intrinsic.getPpx(), intrinsic.getPpy(), intrinsic.getFx(), intrinsic.getFy(),
                    intrinsic.getModel().value(), intrinsic.getCoeffs(), pixels, depths, reference);
            boolean equal = Arrays.equals(reference, xyz);
            android.util.Log.i("SemanticCloud", "CLOUD_VERIFY equal=" + equal + " points=" + count);
            if (!equal) {
                cache[1] = 1;
                xyz = reference;
            }
