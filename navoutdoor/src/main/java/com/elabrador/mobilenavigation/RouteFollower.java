...
final class RouteFollower {
    ...
    synchronized Guidance update(double wgsLatitude, double wgsLongitude, float accuracyMeters, float heading) {
        ...
        // Scale the arrival radius with the GPS fix's own reported accuracy: a fixed 10m/20m
        // threshold is rarely reached by consumer phone GPS (typically 15-30m accuracy), which
        // left manual "结束导航" as the only reliable way to advance to the next phase.
