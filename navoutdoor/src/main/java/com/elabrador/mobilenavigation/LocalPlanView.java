...
public final class LocalPlanView extends View {
    ...
    void setPlan(LocalPlanner.PathResult result) {
        LocalPlanner.PathResult r = result == null ? LocalPlanner.PathResult.waitingForTarget() : result;
        setPlan(r.visualizationGrid, r.waitingReason);
    }
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        int[][] grid = visualizationGrid;
        if (grid == null || grid.length == 0 || grid[0].length == 0) {
            canvas.drawColor(Color.rgb(238, 241, 244));
            String message = waitingReason == null
                    ? "等待局部代价图和 A* 路径" : waitingReason;
        ...
        // Rasterize the immutable 80x80 plan once, then let Canvas draw one bitmap.
        // This keeps the visual result identical while avoiding 6,400 Java draw calls
        // on every real semantic-map refresh.
        if (gridBitmap == null || bitmapRows != rows || bitmapCols != cols) {
            int[] pixels = new int[rows * cols];
            for (int row = 0; row < rows; row++) {
                int display = displayRow(rows, row);
                for (int col = 0; col < cols; col++) {
                    pixels[display * cols + col] = costColor(grid[row][col]);
                }
            }
            gridBitmap = Bitmap.createBitmap(pixels, cols, rows, Bitmap.Config.ARGB_8888);
            bitmapRows = rows;
            bitmapCols = cols;
        }
        canvas.drawBitmap(gridBitmap, null,
