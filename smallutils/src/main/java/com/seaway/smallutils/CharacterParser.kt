...
object CharacterParser {
     ...
     * @return
     */
     fun getChsAscii(chs: String): Int {
        var asc = 0
        try {
     ...
     *
     * @param sortKeyString 拼音
     * 数据库中读取出的sort key
     * @return 英文字母或者#
     */
    fun getSortKey(sortKeyString: String): String {
        val key = sortKeyString.substring(0, 1).toUpperCase(Locale.ROOT)
        return if (key.matches(Regex("[A-Z]")))  key else "#"
    }
    ...
    fun getFirstLetter(chinese: String): String{
        return getSortKey(getSelling(chinese))
    }
}
