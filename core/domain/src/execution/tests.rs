...
fn operation_ref_rejects_blank_components() {
    assert!(CapabilityContractRef::new("", "move", "v1").is_err());
    assert!(CapabilityContractRef::new("mobility", " ", "v1").is_err());
...
fn execution_intent_orders_transport_neutral_parameters() {
    let intent = ExecutionIntent::new(
        CapabilityContractRef::new("navigation", "goto", "v1").expect("operation must be valid"),
        BTreeMap::from([
    ...
    assert_eq!(
        intent.capability_contract().to_string(),
        "navigation.goto@v1"
    );
    assert_eq!(
        intent
...
fn execution_intent_rejects_blank_parameter_key() {
    let result = ExecutionIntent::new(
        CapabilityContractRef::new("system", "ping", "v1").expect("operation must be valid"),
